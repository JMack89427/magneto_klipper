...
class NetworkPanel(ScreenPanel):
    ...
    def __init__(self, screen, title):
        ...
        mag_wifi = self._gtk.Button("light", None, "color1", .66)
        mag_wifi.connect("clicked", self.mag_wifi_help)
        mag_wifi.set_hexpand(False)
    ...
    def mag_wifi_help(self, widget=None):
        logging.debug("Mag wifi help")
        try:
            # 使用subprocess.run调用shell脚本，捕获输出和错误
            result = subprocess.run("/home/pi/auto-uuid/mag_wifi.sh", capture_output=True, text=True, check=True)
            print(f"mag wifi:\n{result.stdout}")
            print(f"mag wifi error\n{result.stderr}")
        except subprocess.CalledProcessError as e:
            print(f"mag wifi script error{e}")
        except Exception as e:
            print(f"mag wifi execut error:{e}")
